...
BaseFloat ComputeDetect_at_10FA(std::vector<BaseFloat> *target_scores,
                     std::vector<BaseFloat> *nontarget_scores,
                     BaseFloat *threshold) {
  KALDI_ASSERT(!target_scores->empty() && !nontarget_scores->empty());
  //std::sort(target_scores->begin(), target_scores->end());
  ...
  size_t target_position = 0, correct_counter =0,
      target_size = target_scores->size();
  size_t threshold_index=0.9*nontarget_scores->size();
  *threshold=(*nontarget_scores)[threshold_index];
  for (; target_position < target_size; target_position++) {
	  if  ((*target_scores)[target_position]>*threshold)
		  ++correct_counter;
  }

  BaseFloat detect = correct_counter * 1.0 / target_size;
  return detect;
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        "Computes Equal Error Rate & detect at 10% false alarm\n"
        ...
        "The EER & detect will be printed to the standard output.\n"
        "\n"
        "Usage: compute-eer_nir <scores-in>\n"
        "e.g.: compute-eer_nir -\n";
    ...
    BaseFloat threshold, threshold_10FA;
    BaseFloat eer = ComputeEer(&target_scores, &nontarget_scores, &threshold);
    BaseFloat detect_at_10FA = ComputeDetect_at_10FA(&target_scores, &nontarget_scores, &threshold_10FA);

    KALDI_LOG << "Equal error rate is " << (100.0 * eer)
              << "%, at threshold " << threshold
			  << "detection rate at 10% FA is " << (100.0 * detect_at_10FA)
			  << "at threshold " << threshold_10FA;

    std::cout.precision(4);
    std::cout << (100.0 * eer) << " "<< (100.0 * detect_at_10FA);
