...
if [ $# != 1 ]; then
  echo "Usage: $(basename $0) [options] <train-dir>"
  echo "e.g.: $(basename $0) --heldout_sent 1000 --arpa --lmtype 3gram-mincount data/lang_test"
  echo "Options:"
  ...
  echo "--threshold-prune    # run prune_lm with threshold"
fi
...
if $run_prune; then
  echo "Prune lm with threshold: $threshold_prune"
  prune_lm.sh --arpa $threshold_prune $subdir
fi
