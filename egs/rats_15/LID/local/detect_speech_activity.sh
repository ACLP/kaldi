...
convert_data_dir_to_whole=false   # If true, the input data directory is 
...
segment_padding=0   # Duration (in seconds) of padding added to segments 
...
work_dir=$4       # Work directory
data_dir=$5       # The output data directory will be ${data_dir}/seg_out

affix=${affix:+_$affix}
feat_affix=${feat_affix:+_$feat_affix}

data_id=`basename $src_data_dir`
sad_dir=${work_dir}/${sad_name}${affix}_${data_id}${feat_affix}
seg_dir=${work_dir}/${segmentation_name}${affix}_${data_id}${feat_affix}
...
if $convert_data_dir_to_whole; then
  sad_dir=${work_dir}/${sad_name}${affix}_${data_id}_whole${feat_affix}
  seg_dir=${work_dir}/${segmentation_name}${affix}_${data_id}_whole${feat_affix}
  ...
  if [ $stage -le 1 ]; then
    steps/make_$pitch_affix.sh --mfcc-config $mfcc_config --nj $nj --cmd "$cmd" --write-utt2num-frames true \
...
mkdir -p $work_dir
if [ $stage -le 4 ]; then
  if [ "$(readlink -f $sad_nnet_dir)" != "$(readlink -f $work_dir)" ]; then
    cp $sad_nnet_dir/cmvn_opts $work_dir || exit 1
  fi
  ...
  if [ ! -z "$output_name" ] && [ "$output_name" != output ]; then
    $cmd $work_dir/log/get_nnet_${output_name}.log \
      nnet3-copy --edits="rename-node old-name=$output_name new-name=output" \
      $sad_nnet_dir/$iter.raw $work_dir/${iter}_${output_name}.raw || exit 1
    iter=${iter}_${output_name}
  else 
    if ! diff $sad_nnet_dir/$iter.raw $work_dir/$iter.raw; then
      cp $sad_nnet_dir/$iter.raw $work_dir/
    fi
  fi
  ...
  steps/nnet3/compute_output.sh --nj $nj --cmd "$cmd" \
    ...
    ${test_data_dir} $work_dir $sad_dir || exit 1
fi
...
graph_dir=${work_dir}/graph_${output_name}
if [ $stage -le 5 ] && [ ! -e $graph_dir/HCLG.fst ]; then
  mkdir -p $graph_dir
...
if [ $stage -le 6 ]; then
  ...
  gunzip -c $seg_dir/ali.*.gz | \
    ...
    gawk '{ $1=$1" ["; print $0 "]" }' | \
    copy-vector ark:- ark,scp:$data_dir/vad.ark,$data_dir/vad.scp

  echo $frame_subsampling_factor > $data_dir/vad_subsampling_factor
fi

echo "$0: Created vad.{scp,ark} in ${data_dir}"
exit 0
# remainder is not needed for x-vector dnn
...
if [ $stage -le 8 ]; then
  utils/data/subsegment_data_dir.sh ${test_data_dir} ${seg_dir}/segments \
    ${data_dir}/seg_out
  cp $src_data_dir/wav.scp ${data_dir}/seg_out
  # cp $src_data_dir/{stm,reco2file_and_channel,glm} ${data_dir}/seg_out/ || true
  utils/fix_data_dir.sh ${data_dir}/seg_out
fi
cp ${data_dir}/seg_out/segments ${data_dir}/vad.segments
echo "$0: Created output segmented kaldi data directory in ${data_dir}/vad.segments"
exit 0
