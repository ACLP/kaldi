...
if [ -f $srcdir/utt2lang ]; then
  utils/apply_map.pl -f 2 $srcdir/utt2lang <$dir/new2old_utt >$dir/utt2lang
fi
...
if [ -f $srcdir/feats.scp ]; then
  ...
  if [ -f $srcdir/vad.scp ]; then
    vad_frame_shift=frame_shift
    if [ -s $srcdir/vad_subsampling_factor ]; then
      vad_frame_shift=$(echo "$frame_shift * $(cat $srcdir/vad_subsampling_factor)" | bc )
      echo "$0: note: VAD frame shift set to $frame_shift [affects vad.scp]"
    fi
    cat $subsegments | awk -v s=$vad_frame_shift '{print $1, $2, int(($3/s)+0.5), int(($4/s)-0.5);}' | \
