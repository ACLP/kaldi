#!/bin/bash
# Ruthi - Original copied from: https://github.com/horndasch/kaldi/tree/d4f70183d3f474f98a8945e028906dd114e94e8a/egs/wsj/s5/local/wclass
...
all_words_txt1=$wclass_dir/all_words1.txt
...
for wclass_label in $( cat $wclass_list | awk '{ print $1 }' ); do
  ...
  utils/afeka/wclass/determine_wclass_specific_counts.pl --replace-blanks-with-dash "true" $wclass $wclass_label \
    $wclass_dir/wclass_list.txt ${wclass_count_file} $wclass_lm_dir/counts.$wclass

	############## Ruthi - replace with fstcompile
...
#  utils/afeka/wclass/convert_to_embeddable_fst.pl | \
#  utils/afeka/wclass/replace_BOS_and_EOS_with_disambig_symbol.pl \
...
#  fstcompile -isymbols=$all_words_txt -osymbols=$all_words_txt --keep_osymbols=false > #$wclass_lm_dir/$wclass.fst
	awk '{print $2,$2}' $all_words_txt > $all_words_txt1
	fstcompile --isymbols=$all_words_txt1 --osymbols=$all_words_txt1 --keep_isymbols --keep_osymbols $wclass_dir/$wclass.txt $wclass_lm_dir/$wclass.fst
...
##############End Ruthi
  ...
  if grep -q "${wclass_label}_SWU" $wclass_lm_dir/counts.$wclass; then
    ...
    arpa2fst - | fstminimize | fstrmepsilon | fstprint | \
    utils/afeka/wclass/convert_to_embeddable_fst.pl --back-transition "#${wclass}_SWU_BACK" | \
    utils/afeka/wclass/replace_BOS_and_EOS_with_disambig_symbol.pl \
    ...
    if [ -n "$swu_model_label" ]; then
	  ...
	  cp $wclass_lm_dir/$wclass.fst $wclass_lm_dir/$wclass.old.fst
