...
max_average=0.95
#online_ivector_dir=
sup_online_ivector_dir=
unsup_online_ivector_dir=
...
frames_per_eg=1 # to be passed on to get_egs2_semi_supervised.sh
weight_threshold=0.7    # Only select frames above this score from unsupervised data
supervised_copies=3     # Make x copies of supervised data to "over" weight supervised data
...
echo $@
if [ $# != 6 ]; then
  echo "Usage: $0 [opts] <data-sup> <data-unsup> <lang> <sup-ali-dir> <unsup-lat-dir> <exp-dir>"
  echo " e.g.: $0 data/train-sup data/train-unsup data/lang exp/tri3_ali exp/tri3/decoder_unsup exp/tri4_nnet"
  echo ""
...
# Check some files.
for f in $data_sup/feats.scp $data_unsup/feats.scp $lang/L.fst $alidir/ali.1.gz $alidir/final.mdl $alidir/tree; do
...
lda_extra_opts=()
[ ! -z "$cmvn_opts" ] && lda_extra_opts+=(--cmvn-opts "$cmvn_opts")
[ ! -z "$feat_type" ] && lda_extra_opts+=(--feat-type $feat_type)
[ ! -z "$sup_online_ivector_dir" ] && lda_extra_opts+=(--online-ivector-dir $sup_online_ivector_dir)   # Required for get_lda.sh !!!
[ -z "$transform_dir" ] && transform_dir=$alidir
lda_extra_opts+=(--transform-dir $transform_dir)

if [ $stage -le -4 ]; then
  echo "$0: calling get_lda.sh  >>>>> Running on supervised data only!!!!"
  steps/nnet2/get_lda.sh $lda_opts "${lda_extra_opts[@]}" --left-context $first_left_context --right-context $first_right_context --cmd "$cmd" $data_sup $lang $alidir $dir || exit 1;
fi

extra_opts=()
[ ! -z "$cmvn_opts" ] && extra_opts+=(--cmvn-opts "$cmvn_opts")
[ ! -z "$feat_type" ] && extra_opts+=(--feat-type $feat_type)
[ ! -z "$sup_online_ivector_dir" ] && extra_opts+=(--sup-online-ivector-dir $sup_online_ivector_dir)
[ ! -z "$unsup_online_ivector_dir" ] && extra_opts+=(--unsup-online-ivector-dir $unsup_online_ivector_dir)
#[ -z "$transform_dir" ] && transform_dir=$alidir
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  echo "$0: calling get_egs2_semi_supervised.sh"

  utils/afeka/online/get_egs2_semi_supervised.sh $egs_opts "${extra_opts[@]}" \
      --samples-per-iter $samples_per_iter --stage $get_egs_stage \
      --io-opts "$io_opts" \
      --cmd "$cmd" $egs_opts \
      --frames-per-eg $frames_per_eg \
      --weight-threshold $weight_threshold\
      --supervised-copies $supervised_copies\
	  --do-supervised-semisup-train $do_supervised_semisup_train\
      $data_sup $data_unsup $alidir $latdir $dir/egs || exit 1;
fi
...
if [ $stage -le -1 ]; then
  echo "Training transition probabilities and setting priors - Using supervised data only !!!"
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    if [ ! -z "${realign_this_iter[$x]}" ]; then
      ...
      echo ">>>>Realignment not supported in train_multisplice_accel2_semisup.sh !!!!<<<<<<"
      echo ">>>>Realignment not supported in train_multisplice_accel2_semisup.sh !!!!<<<<<<"
      echo ">>>>Realignment not supported in train_multisplice_accel2_semisup.sh !!!!<<<<<<"
      exit 1
      
      steps/nnet2/align.sh --nj $num_jobs_align --cmd "$align_cmd" --use-gpu $align_use_gpu \
        --transform-dir "$transform_dir" --online-ivector-dir "$online_ivector_dir" \  ##Ella
        --iter $x $data $lang $dir $dir/ali_$time || exit 1 #Ella
