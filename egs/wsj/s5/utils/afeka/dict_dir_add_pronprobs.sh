...
# based utils/dict_dir_add_pronprobs.sh
# creates lexiconp.txt from pron_counts and for new words add-one smoothing.
...
awk "BEGIN { while ((getline < \"$dir/lexicon.txt\")>0) lex[\$1] = 1} { if (\$2 in lex) print \$0 }" $pron_counts > $dir/pron_counts

cat <(awk '{print 1, $0;}' <$dir/lexicon.txt) $dir/pron_counts | \
