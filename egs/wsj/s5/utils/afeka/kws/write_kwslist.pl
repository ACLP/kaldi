...
Allowed options:
  ...
  --reco2fc                   : reco2file_and_channel file from Kaldi       (string,  default = "")
...
GetOptions('segments=s'     => \$segment,
  'reco2fc=s'     => \$reco2file_and_channel,
  'flen=f'         => \$flen,
  'beta=f'         => \$beta,
  'duration=f'     => \$duration,
  'language=s'     => \$language,
...
if ($reco2file_and_channel) {
  open(RECO2FC, "<$reco2file_and_channel") || die "$0: Fail to open file and channel table $reco2file_and_channel\n";
}
...
if ($reco2file_and_channel) {
  while (<RECO2FC>) {
    chomp;
    my @col = split(" ", $_);
    @col == 3 || die "$0: Bad number of columns in $reco2file_and_channel \"$_\"\n";
    $audio_mapper{$col[0]} = $col[1];
    $channel_mapper{$col[0]} = $channel_names{$col[2]};
    #print "audio_id=$col[0]; file=$col[1]; channel=$col[2]\n";
  }
}
...
while (<$source>) {
  ...
  if ($reco2file_and_channel) {
    $audio = $audio_mapper{$utter};
    $channel = $channel_mapper{$utter};
  } else {
    $audio = $utter;
  }
  push(@KWS, [$kwid, $audio, $channel, $start, $dur, $score, ""]);
}
...
if ($reco2file_and_channel) {close(RECO2FC);}
