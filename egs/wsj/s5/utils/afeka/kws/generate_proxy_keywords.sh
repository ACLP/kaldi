...
if [ $# -ne 1 ]; then
  ...
  echo ""
  echo "Usage: $(basename $0) <kws-data-dir>"
  echo " e.g.: $(basename $0) data/dev10h/kws_proxy/"
  exit 1;
fi
...
if [ ! -z $confusion_matrix ]; then
  ...
  utils/afeka/count_to_logprob.pl --cutoff $count_cutoff \
...
cat $kwsdatadir/phones.txt |\
  ...
  utils/afeka/build_edit_distance_fst.pl --boundary-off=true \
  $confusion_matrix_param - - |\
  fstcompile --isymbols=$kwsdatadir/phones.txt \
