...
# based utils/kwslist_post_process.pl
...
sub ReadKwslist {
  ...
  while (<$source>) {
    ...
    if (/<kw/) {
      ...
      ($score) = /score="(\S+)"/;
      ($channel) = /channel="(\S+)"/;
      push(@KWS, [$kw2raw{$kwid}, $file, $channel, $tbeg, $dur, $score, ""]);
...
sub PrintKwslist {
  ...
  # Start printing
  $kwslist .= "<kwslist kwlist_filename=\"$info->[0]\" language=\"$info->[1]\">\n";
  my $prev_kw = "";
  foreach my $kwentry (@{$KWS}) {
    if ($prev_kw ne $kwentry->[0]) {
      if ($prev_kw ne "") {$kwslist .= "  </detected_kwlist>\n";}
      $kwslist .= "  <detected_kwlist kwid=\"$kwentry->[0]\">\n";
      $prev_kw = $kwentry->[0];
    }
...
my $Usage = <<EOU;
This script reads a keywords_raw.txt and kwslist.xml results file,does the post processing such as making decisions,
normalizing score, removing duplicates, etc. It writes the results to another kwslist.xml file.

Usage: kwslist_post_process.pl [options] <keywords_raw_in> <kwslist_in|-> <kwslist_out|->
...
my $kwslist_raw_in = shift @ARGV;
my $kwslist_in = shift @ARGV;
my $kwslist_out = shift @ARGV;

open(S, "<$kwslist_raw_in") || die "opening segments file $kwslist_raw_in";
while(<S>) {
  my ($kw_id, $kw_raw) = split(" ", $_);
#  print STDERR $kw_id."---".$kw_raw."\n";
  $kw2raw{$kw_id} = $kw_raw;
}
close(S);
