...
# /storage/kaldi-trunk/tools/kaldi_lm/train_lm.sh

# This script trains an LM, using a technique closely related
# to modified, interpolated Kneser-Ney, but which performs a bit better.
# The LM is stored in a "Kaldi-LM" format, as N-gram counts; 
# or with the --arpa option, in the ARPA format.

# The input is a directory where there should exist the following
# files, look at scripts that call this script for examples of how to obtain these
# train.gz  [the training set in "mapped form"]
# dev.gz  [the dev set in "mapped form"]
...
# for example, to estimate discounting factors).
# For now its size is hardwired at 10k sentences.
...
if [ $# != 2 ]; then
  echo "Usage: $(basename $0) [options] <text-dir> <lang-dir>"
  echo "e.g.: $(basename $0) --arpa --lmtype 3gram-mincount data/lang_test/data data/lang_test"
  echo "Options:"
  echo "main options (for others, see top of script file)"
  echo "--arpa               # write arpa file as well as N-gram format ( default false)"
...
if [ -s $subdir/train_ngrams.gz -a -s $subdir/dev_ngrams.gz ]; then
  echo "Not creating raw N-gram counts since they already exist in $subdir"
...
EOF
    gunzip -c $text_dir/train.gz | \
     ...
     sort | merge_ngrams | gzip -c > $subdir/train_ngrams.gz
    gunzip -c $text_dir/dev.gz | \
     get_raw_ngrams 3 | sort | uniq -c | uniq_to_ngrams | \
     perl -ane 's/(\S+)$/:$1/; print;' | sort | gzip -c > $subdir/dev_ngrams.gz
    ;; 
...
[ ! -f $subdir/train_ngrams.gz -o ! -s $subdir/train_ngrams.gz ] && \
...
get_perplexity()  { # echoes the perplexity to stdout. uses current "$config" as config
  time gunzip -c $subdir/train_ngrams.gz | \
   discount_ngrams "$config" | sort | merge_ngrams | \
   interpolate_ngrams $dir/wordlist.mapped 0.5 | sort | \
   sort -m <(gunzip -c $subdir/dev_ngrams.gz) - | compute_perplexity
}
...
if gunzip -c $subdir/ngrams_disc.gz >&/dev/null; then
  ...
  gunzip -c $subdir/train_ngrams.gz | \
...
gunzip -c $subdir/ngrams_disc.gz | \
  ...
  sort | sort -m <(gunzip -c $subdir/dev_ngrams.gz) - | \
